...
function makeMove(row, col) {
	...
	statusElement.textContent = currentPlayer === "◯" ? "あなた（◯）の番です" : "コンピュータ（✕）の番です";
...
function getMetaWinningSequence(player) {
	for (let i = 0; i < 3; i++) {
		if (metaBoard[i][0] === player && metaBoard[i][1] === player && metaBoard[i][2] === player) {
			return [{r: i, c: 0}, {r: i, c: 1}, {r: i, c: 2}];
		}
		if (metaBoard[0][i] === player && metaBoard[1][i] === player && metaBoard[2][i] === player) {
			return [{r: 0, c: i}, {r: 1, c: i}, {r: 2, c: i}];
		}
	}
	if (metaBoard[0][0] === player && metaBoard[1][1] === player && metaBoard[2][2] === player) {
		return [{r: 0, c: 0}, {r: 1, c: 1}, {r: 2, c: 2}];
	}
	if (metaBoard[0][2] === player && metaBoard[1][1] === player && metaBoard[2][0] === player) {
...
function checkMiniWin(miniRow, miniCol) {
	let startRow = miniRow * 3, startCol = miniCol * 3;
	let mini = [];
	for (let i = 0; i < 3; i++) {
		mini[i] = [];
		for (let j = 0; j < 3; j++) {
...
function displayLargeMark(miniRow, miniCol, player) {
	...
	metaDiv.innerHTML = `<div class="large-mark" style="color:${color}">${player}</div>`;
}
...
function displayWinningLine(type, sequence) {
	...
	if (type === "overall") {
		...
		let extension = 25;
		let first = sequence[0], last = sequence[sequence.length - 1];
		...
		let unitX = deltaX / length, unitY = deltaY / length;
...
function cloneState(state) {
    return {
        ...
        secondLastMove: state.secondLastMove ? { ...state.secondLastMove } : null,
...
function computerMove() {
    ...
    let currentState = {
        ...
        secondLastMove: secondLastMove ? { ...secondLastMove } : null,
        gameOver: gameOver
    };
    let { move } = minimax(currentState, SEARCH_DEPTH, -Infinity, Infinity, true);
    if (move) {
